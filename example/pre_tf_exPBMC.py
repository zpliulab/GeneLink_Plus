...
def pre_data():
    base_dir = ''
    save_dir = 'dataset\hTFTarget\B\TFs+500'
    TF_Target_information_dir = os.path.join(base_dir, 'dataset_PBMC', 'B_GRN.csv')
    expression_matrix_path = os.path.join(base_dir, 'dataset_PBMC', 'B_ex.csv')
    ...
    df_filtered = df_filtered[
        df_filtered['TF'].isin(genes_in_expression_df) & df_filtered['Target'].isin(genes_in_expression_df)].copy()
    ...
    all_genes_df = all_genes_df.sort_values('Gene').reset_index(drop=True)
    all_genes_df['index'] = range(len(all_genes_df))

    # Apply mappings for TF and Target indices
    df_filtered['TF_index'] = df_filtered['TF'].apply(
        lambda x: all_genes_df.loc[all_genes_df['Gene'] == x, 'index'].values[0])
    df_filtered['Target_index'] = df_filtered['Target'].apply(
        lambda x: all_genes_df.loc[all_genes_df['Gene'] == x, 'index'].values[0])
